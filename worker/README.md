...
- **הקישור לא פג ואפשר להשתמש בו יותר מפעם אחת.** אין דאטהבייס, ולכן אין רישום
  שהקישור כבר נוצל. מי שמחזיק בקישור יכול לחתום שוב בשם אחר ולייצר עותק חתום נוסף.
  הנזק מוגבל — הנמענים נעולים לכתובת המשרד ולכתובות הצדדים שבתוך ה-payload החתום —
  אבל זו מגבלה אמיתית. לסגירה מלאה צריך KV שירשום קישורים שנוצלו ובדיקה ב-`handleSign`.
  להקטנת החלון בלי אחסון: הוסף `exp` ל-payload ובדוק אותו ב-`handleSign`.
- **חתימת בעל נכס שני.** אם באינטייק סומן co-owner, אחרי שהבעלים הראשון חותם
  ה-Worker שולח אוטומטית קישור חדש לבעלים השני, ורק אחרי שגם הוא חותם החוזה
  מסומן FULLY EXECUTED ונשלח לכל הצדדים.
